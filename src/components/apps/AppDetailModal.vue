...
import type { AppExecutionLog } from '../../stores/appCenter'
...
<template>
  <a-modal
    ...
    :width="1100"
    class="shadow-2xl rounded-2xl overflow-hidden"
    @cancel="handleClose"
  >
    <template #title>
      <div v-if="app" class="flex items-center justify-between w-full pr-6">
        <div class="flex items-center gap-3">
          <span
            class="flex size-11 items-center justify-center rounded-xl bg-slate-100 text-2xl text-[#165dff] shadow-sm"
          >
          ...
          <div>
            <div class="flex items-center gap-2">
              <h2 class="text-lg font-bold text-slate-900">{{ app.name }}</h2>
        ...
        <div class="flex items-center gap-2">
          <a-button type="outline" size="medium" class="!rounded-lg" @click="handleShare">
          ...
          <a-button
            ...
            type="primary"
            size="medium"
            class="!rounded-lg !bg-rose-500 hover:!bg-rose-600 !px-5"
          ...
          <template v-else>
            <a-button
              type="primary"
              size="medium"
              class="!rounded-lg !bg-emerald-600 hover:!bg-emerald-700 !px-4"
    ...
    <div v-if="app" class="min-h-[620px] flex flex-col">
      <!-- Top Navigation Tabs -->
      <a-tabs v-model:active-key="activeTab" type="rounded" class="app-detail-tabs mb-4">
      ...
      <div v-else-if="activeTab === 'config'" class="space-y-6 max-h-[540px] overflow-y-auto pr-1">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4">
          <div class="flex items-center justify-between mb-3">
            ...
            <a-button type="text" size="mini"><template #icon><IconPlus /></template>添加输入字段</a-button>
          </div>
          <a-table :data="app.inputs" :pagination="false" size="small" border>
            <template #columns>
              <a-table-column title="字段 Key" data-index="key" :width="150" />
              <a-table-column title="显示名称" data-index="label" :width="160" />
              <a-table-column title="控件类型" data-index="type" :width="120">
              ...
              <a-table-column title="默认值" data-index="defaultValue" :width="180">
                <template #cell="{ record }">
            ...
            <a-table :data="app.permissions.roles" :pagination="false" size="small" border>
              <template #columns>
                ...
                <a-table-column title="操作权限等级" data-index="access">
                  <template #cell="{ record }">
                    <a-select v-model="record.access" size="small" class="w-36">
    ...
    <a-modal v-model:visible="addTriggerModalVisible" title="添加新触发器" @ok="handleAddTrigger">
      <a-form :model="{ newTriggerName, newTriggerType, newTriggerConfig }" layout="vertical">
        <a-form-item label="触发器名称" required>
          <a-input v-model="newTriggerName" placeholder="例如：工作日下班定时轮询" />
        </a-form-item>
        <a-form-item label="触发器类型">
          <a-select v-model="newTriggerType">
        ...
        <a-form-item label="规则表达式 / 事件描述">
          <a-input v-model="newTriggerConfig" placeholder="0 18 * * 1-5 或 订单支付成功事件" />
        </a-form-item>
      </a-form>
    </a-modal>
...
<style scoped>
.app-detail-tabs :deep(.arco-tabs-nav-tab) {
